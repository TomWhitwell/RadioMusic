...
void select_new_firmware(){
	...
	/* stop playing RM otherwise it might mess the sdcard */
	app_stop();
	
	/* called rewindDirectory for reinit pointer to root folder */
	File root = SD.open("/");
...
//					/* make the leds blinks */
		...
		/* update the current firmware file */
//			upd_current_firmware_file(selected);
	}
}
...
//String init_reset_mode(){
//	char c;
//	File f;
//	String str;
//	if(SD.exists("firmware/current.txt")){
//		f = SD.open("firmware/current.txt");
//		if(f){
//			while(f.available()){
//					c = f.read();
//					str += c;
...
void setup(){
	// call initialization function of RadioMusic
